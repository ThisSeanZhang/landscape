...
const noticeText = computed(() => t("geo_editor.geo_site.adguard_limit_notice"));
...
const supportedRules = computed<RuleRow[]>(() => [
  ...
  { rule: "||domain^$important / $document",    reason: desc("supported_2") },
  { rule: "0.0.0.0 domain / 127.0.0.1 domain / :: domain", reason: desc("supported_3") },
  { rule: "|https://domain|",                   reason: desc("supported_4") },
  { rule: "domain.com",                         reason: desc("supported_5") },
]);

const skippedRules = computed<RuleRow[]>(() => [
  { rule: "@@...",                                                  reason: desc("skipped_1") },
  { rule: "$third-party / $3p / $domain= / $denyallow= / $to= / $client= / $dnstype=", reason: desc("skipped_2") },
  { rule: "||domain.com/path^",                                     reason: desc("skipped_3") },
...
<template>
  <n-popover trigger="hover">
    ...
    <div style="max-width:340px;font-size:12px;cursor:pointer" @click="show = true">
      <n-text depth="2">{{ noticeText }}</n-text>
      <n-text type="info" depth="3" style="display:block;margin-top:4px">{{ sup("click_hint") }}</n-text>
    </div>
  </n-popover>
    ...
    <n-card size="small" closable :title="sup('title')" @close="show = false">
      <n-flex vertical :size="10">
        <n-flex vertical :size="4">
          <n-text type="success" strong style="font-size:13px">{{ sup("supported_title") }}</n-text>
        ...
        <n-flex vertical :size="4">
          <n-text type="error" strong style="font-size:13px">{{ sup("skipped_title") }}</n-text>
          <n-table :bordered="false" size="small">
            <thead>
              <tr>
                <th style="width:200px">{{ sup("col_rule") }}</th>
