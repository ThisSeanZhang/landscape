...
pub fn parse_link_message(msg: LinkMessage) -> Option<LandscapeInterface> {
    ...
    for nla in msg.attributes.into_iter() {
        ...
        match nla {
            ...
            LinkAttribute::IfNetnsId(id) => netns_id = Some(id),
    ...
    match name {
        Some(name) => {
            ...
            Some(LandscapeInterface {
                ...
                controller_id,
                dev_status: status.map_or(DevState::Unknown, convert_link_state),
                dev_kind: kind.map_or(DeviceKind::UnKnow, convert_link_kind),
                perm_mac,
                carrier,
                netns_id,
                peer_link_id,
